...
StringFormatter = {
  ...
  autoLink : function() {
    $('.autolink:not(.linked)').each(function(index, item) {
      ...
      $(result.match(/(https?.*?).*$/gm)).each( function(index, element) {
        var caption = element.match(/^((http[s]?|ftp):\/)?\/?([^:\/\s]+)((\/\w+)*\/)([\w\-\.]+[^#?\s]+)(.*)?(#[\w\-]+)?$/);
        var link = "<a href='" + element + "'>" + caption[0] + "</a>";
        result = result.replace(element, link);
